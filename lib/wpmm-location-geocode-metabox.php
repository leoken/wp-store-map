...
function wpmm_mbe_function( $post ) {
	...
	<p>
		<?php _e( 'Phone:', 'wpmm-map-markers' ); ?> 
		<input type="text" class="widefat" id="phone" name="phone" value="<?php echo esc_attr( $phone ); ?>" />
	</p>
	<p>
		<?php _e( 'Email:', 'wpmm-map-markers' ); ?> 
		<input type="text" class="widefat" id="email" name="email" value="<?php echo esc_attr( $email ); ?>" />
	</p>
...
function wpmm_mbe_save_meta( $post_id ) {
	...
	if ( isset( $_POST['phone'] ) ) {
		...
		//save the meta data
		update_post_meta( $post_id, 'phone', strip_tags( $_POST['phone'] ) );
	}
